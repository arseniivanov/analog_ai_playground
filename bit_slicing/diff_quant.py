...
#LEGACY below, more hardware-like but not easy to train


class STEQuantizeDual(autograd.Function):
    ...
    def forward(ctx, input, bitwidth):
        ...
        positive_weights = torch.clamp(input, 0, 1)
        negative_weights = torch.clamp(input, -1, 0)
        ...
        positive_weights_quantized = torch.round(positive_weights * (bitwidth - 1) / 2) / ((bitwidth - 1) / 2)
        ...
        negative_weights_quantized = torch.round(negative_weights * (bitwidth - 1) / 2) / ((bitwidth - 1) / 2) - 1

        return positive_weights_quantized, negative_weights_quantized
    
    @staticmethod
    def backward(ctx, grad_output_pos, grad_output_neg):
        # For now, we'll use STE for both positive and negative weights.
        # We also need to return a None gradient for the second input (bitwidth), as it's not trainable.
        return grad_output_pos + grad_output_neg, None
...
class DifferentialQuantizedLayer(nn.Module):
    ...
    def forward(self, x):
        pos_weights, neg_weights = STEQuantizeDual.apply(self.layer.weight, torch.tensor(self.bitwidth, dtype=torch.int))
